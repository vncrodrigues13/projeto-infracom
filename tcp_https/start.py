...
class QuizRunner:
    ...
    def run_server(self):
        print("🖥️  Iniciando servidor HTTPS...")
        try:
            self.server_process = subprocess.Popen(
                [sys.executable, "servidor_https.py"],
            ...
            print(f"[SERVIDOR HTTPS] Processo iniciado com PID: {self.server_process.pid}")
    ...
    def run_client_instance(self, client_id):
        try:
            process = subprocess.Popen(
                [sys.executable, "clientes_https.py"],
            ...
            print(f"✅ Cliente HTTPS {client_id} iniciado (PID: {process.pid})")
            
            process.wait()
                    
        except Exception as e:
            print(f"❌ Erro no cliente HTTPS {client_id}: {e}")

    def run_multiple_clients(self):
        print("⏳ Aguardando servidor HTTPS inicializar...")
        time.sleep(5)
        
        print(f" Iniciando {self.num_clients} clientes HTTPS em paralelo...")
        ...
        print("✅ Todos os clientes HTTPS iniciados!")
    ...
    def extract_final_results(self):
        print("\n" + "="*60)
        print(" RESULTADO FINAL DO JOGO HTTPS")
        print("="*60)
        ...
        print("\n" + "="*60)
        print("🎯 JOGO HTTPS FINALIZADO!")
        print("="*60)

    def signal_handler(self, sig, frame):
        print("\n Encerrando sistema HTTPS...")
    ...
    def main(self):
        print("🎯 SISTEMA DE QUIZ HTTPS - EXECUÇÃO SIMULTÂNEA")
        print("=" * 60)
        print("📋 Este script executa automaticamente:")
        print(f"   • Servidor HTTPS (servidor_https.py) - Output visível")
        ...
        if not os.path.exists("servidor_https.py"):
            print("❌ Arquivo servidor_https.py não encontrado!")
            return
        if not os.path.exists("clientes_https.py"):
            print("❌ Arquivo clientes_https.py não encontrado!")
            return
