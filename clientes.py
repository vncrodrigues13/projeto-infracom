...
def simulate_client(client_id):
    try:
        ...
        print(f"[CLIENT {client_id}] Conectado ao servidor.")

        while True:
            question = client_socket.recv(BUFFER_SIZE).decode()
            if not question:
        ...
        print(f"[CLIENT {client_id}] Não foi possível conectar ao servidor.")
    finally:
        client_socket.close()
